...
class FirebaseMedicineRepository: MedicineRepositoryProtocol {
    ...
    func searchMedicines(query: String) async throws -> [Medicine] {
        guard !query.isEmpty else {
            return try await getMedicines()
        }
        
        let lowercasedQuery = query.lowercased()
        let medicines = try await getMedicines()
        
        return medicines.filter { medicine in
            medicine.name.lowercased().contains(lowercasedQuery) ||
            ((medicine.reference?.lowercased().contains(lowercasedQuery)) != nil) ||
            (medicine.description?.lowercased().contains(lowercasedQuery) ?? false)
        }
    }
